...
from ..core import get_node_from_graph, default_graph
...
class Saver(object):
    """模型、计算图保存和加载工具类
    模型保存为两个单独的文件：1. 计算图自身的结构元信息（JSON格式）；2. 节点的值，更确切的说是变量节点的权值（二进制格式，使用numpy提供的接口）
    """
    ...
    def _restore_nodes(self, graph, from_model_json, from_weights_dict):
        ...
        for index in range(len(from_model_json)):
            ...
            # 判断是否创建了当前节点，如果已存在，更新其权值；否则，创建节点
