...
predict = vf.operator.Step(output)

# 损失函数
loss = vf.loss.loss.PerceptionLoss(vf.operator.MatMul(label, output))

# 学习率
learning_rate = 0.01

# 使用各种优化器
# optimizer = vf.optimizer.GradientDescent(vf.default_graph, loss, learning_rate)
# optimizer = vf.optimizer.Momentum(vf.default_graph, loss, learning_rate)
# optimizer = vf.optimizer.AdaGrad(vf.default_graph, loss, learning_rate)
# optimizer = vf.optimizer.RMSProp(vf.default_graph, loss, learning_rate)
