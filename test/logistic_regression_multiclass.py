...
x = vf.core.Variable(dim=(4, 1), init=False, trainable=False)

# One-Hot类别标签，是3x1矩阵，不需要初始化，不参与训练
one_hot = vf.core.Variable(dim=(3, 1), init=False, trainable=False)

# 权值矩阵，是一个3x4矩阵，需要初始化，参与训练
W = vf.core.Variable(dim=(3, 4), init=True, trainable=True)

# 偏置向量，是一个3x1矩阵，需要初始化，参与训练
b = vf.core.Variable(dim=(3, 1), init=True, trainable=True)

# 线性部分
linear = vf.operator.Add(vf.operator.MatMul(W, x), b)

# 模型输出
predict = vf.operator.SoftMax(linear)

# 交叉熵损失
loss = vf.loss.CrossEntropyWithSoftMax(linear, one_hot)
