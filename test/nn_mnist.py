...
x = vf.core.Variable(dim=(784, 1), init=False, trainable=False)

# One-Hot类别标签，是10x1矩阵
one_hot = vf.core.Variable(dim=(10, 1), init=False, trainable=False)

# 输入层，100个神经元，激活函数为ReLU
hidden_1 = vf.layer.fc(x, 784, 100, "ReLU")

# 隐藏层，20个神经元，激活函数为ReLU
hidden_2 = vf.layer.fc(hidden_1, 100, 20, "ReLU")

# 输出层，10个神经元，无激活函数
output = vf.layer.fc(hidden_2, 20, 10, None)

# 概率输出
predict = vf.operator.SoftMax(output)

# 交叉熵损失
loss = vf.loss.CrossEntropyWithSoftMax(output, one_hot)
